...
describe('TransferredMandates Code Samples', () => {
  ...
  test('transferred_mandates code sample executes without error', async () => {
    // Convert :param placeholders to regex wildcards for flexible matching
    const stubUrl = '/transferred_mandates/:identity';
    ...
    const responseBody = { transferred_mandates: {} };
    for (let i = 0; i < 5; i++) {
    ...
    try {
      ...
      const transferredMandate = await client.transferredMandates.transferredMandates('MD123');

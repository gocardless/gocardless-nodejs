...
describe('ScenarioSimulators Code Samples', () => {
  ...
  test('run code sample executes without error', async () => {
    // Convert :param placeholders to regex wildcards for flexible matching
    const stubUrl = '/scenario_simulators/:identity/actions/run';
    ...
    const responseBody = { scenario_simulators: {} };
    for (let i = 0; i < 5; i++) {
    ...
    try {
      ...
      const response = await client.scenarioSimulators.run('payment_failed', {
        links: {
          resource: 'PM123',
        },
      });

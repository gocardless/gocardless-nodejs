...
describe('FundsAvailabilities Code Samples', () => {
  ...
  test('check code sample executes without error', async () => {
    // Convert :param placeholders to regex wildcards for flexible matching
    const stubUrl = '/funds_availability/:identity';
    ...
    const responseBody = { funds_availability: {} };
    for (let i = 0; i < 5; i++) {
    ...
    try {
      ...
      const resp = await client.fundsAvailabilities.check('MD123', {
        amount: '1000',
      });

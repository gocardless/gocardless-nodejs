...
describe('CurrencyExchangeRates Code Samples', () => {
  ...
  test('list code sample executes without error', async () => {
    ...
    const stubUrl = '/currency_exchange_rates';
    ...
    const responseBody = { currency_exchange_rates: [{}], meta: { cursors: {}, limit: 50 } };
    for (let i = 0; i < 5; i++) {
    ...
    try {
      ...
      const filteredRates = await client.currencyExchangeRates.list({ source: 'EUR', target: 'GBP' });

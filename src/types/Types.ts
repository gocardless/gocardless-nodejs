...
export type BankAuthorisation = {
  ...
  //  or [`BANK_AUTHORISATION_FAILED`](#billing-request-bankauthorisationfailed)
...
export type BillingRequestCustomerBillingDetail = {
  ...
  //  For ACH customers only. Required for ACH customers. A string containing
  //  the IP address of the payer to whom the mandate belongs (i.e. as a result
  //  of their completion of a mandate setup flow in their browser).
  //
...
export type BillingRequestAction = {
  ...
  //  needed. Either because it was manually selected or the billing request
  //  does not support this feature
  //  - `pending`: we are waiting on the bank details in order to infer the
  //  institution
  //  - `failed`: we weren't able to infer the institution
  //  - `success`: we inferred the institution and added it to the resources of
  //  a Billing Request
  //
  institution_guess_status?: BillingRequestActionInstitutionGuessStatus;

  //  Informs you whether the action is required to fulfil the billing request
  //  or not.
  required?: boolean;
...
export type BillingRequestLinks = {
  ...
  //  (Optional) ID of the [subscription](#core-endpoints-subscriptions) that
  //  was created from this subscription request.
...
export type BillingRequestMandateRequestConstraintsPeriodicLimit = {
  ...
  //  example this will expire on the Monday for the current week or the January
  //  for the next year.
  //
...
export type BillingRequestPaymentRequest = {
  ...
  //  The amount to be deducted from the payment as an app fee, to be paid to
  //  the partner integration which created the billing request, in the lowest
  //  denomination for the currency (e.g. pence in GBP, cents in EUR).
  app_fee?: string | null;
...
export type BillingRequestFlow = {
  ...
  //  not want payers to be automatically redirected or on Android devices,
...
export type BillingRequestWithActionActionsCollectBankAccount = {
  ...
  //  Bank account number - see [local details](#appendix-local-bank-details)
...
export type Creditor = {
  ...
  //  a partner app, and this value is true, you should not send notifications
...
export type EventDetails = {
  ...
  //  <ul>
  //  <li>`failure_filter_applied`: The payment won't be intelligently retried
  //  as
  ...
  //  received in the report from the customer's bank. See the [GoCardless
  //  Direct Debit
...
export type EventLinks = {
  ...
  //  This is only included for mandate transfer events, when it is the ID of
  //  the [customer bank account](#core-endpoints-customer-bank-accounts) which
  //  the mandate is being transferred to.
  new_customer_bank_account?: string;

  //  This is only included for mandate replaced events, when it is the ID of
  //  the new [mandate](#core-endpoints-mandates) that replaces the existing
  //  mandate.
  new_mandate?: string;
  ...
  //  If `resource_type` is `billing_requests`, this is the ID of the
  //  [payment](#core-endpoints-payments) which has been created for Instant
  //  Bank Payment.
  payment_request_payment?: string;
  ...
  //  This is only included for mandate transfer events, when it is the ID of
  //  the [customer bank account](#core-endpoints-customer-bank-accounts) which
  //  the mandate is being transferred from.
...
export type MandateImportEntryAmendment = {
  //  The creditor identifier of the direct debit originator. Required if
  //  mandate
...
export type MandateImportEntryCustomer = {
  //  The first line of the customer's address. Required if mandate import
  //  scheme is either `bacs` or `sepa`.
  //
  address_line1?: string;
...
export enum MandateImportEntryMandateAuthorisationSource {
  Web = 'web',
  Telephone = 'telephone',
  Paper = 'paper',
}
...
export type MandatePdfCreateRequestLinks = {
  ...
  //  from. The customer's bank details will be censored in the generated PDF.
  //  No other parameters may be provided alongside this.
  mandate?: string;
};
...
export type PaymentFx = {
  ...
  //  This will vary based on the prevailing market rate until the moment that
  //  it is paid out.
...
export type PaymentLinks = {
  ...
  //  payment was created.<br/>_Note_: this property will only be present if
  //  this payment is part of a subscription.
  subscription?: string;
};
...
export type PayoutItem = {
  ...
  //  bank or customer cancelled the chargeback. Will include taxes if
...
export type PayoutItemLinks = {
  //  Unique identifier, beginning with "MD". Note that this prefix may not
  //  apply to mandates created before 2016. Present only for the items of type
...
export type RedirectFlowLinks = {
  ...
  //  create a billing request in the event the redirect flow is eligible to
  //  send the payer down this new and improved flow
  billing_request?: string;
...
export type ScenarioSimulator = {
  ...
  //  the `pending_submission` state, and its mandate must be in the `activated`
  ...
  //  having been submitted to the banks, set up successfully and then moved to
  //  a new bank account due. It must start in the `pending_submission` state.
  //  Only compatible with Bacs and SEPA mandates.</li>

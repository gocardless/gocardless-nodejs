...
export class PaymentAccountTransactionService {
  ...
  public async list(
    identity: string,
    requestParameters?: Partial<PaymentAccountTransactionListRequest>,
  ...
  public async *all(
    ...
    do {
      let list = await this.list(identity, { ...requestParameters, after: cursor }, customHeaders);

      for (let paymentaccounttransaction of list.payment_account_transactions) {
